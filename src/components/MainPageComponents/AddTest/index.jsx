...
function NewTest({ setButtonValue }) {
	...
	useEffect(() => {
		...
		api.getCategories(auth).then((response) => setCategories(response.data));

		api.getDisciplines(auth).then((response) => setDisciplines(response.data));
	...
	function handleSelectedDiscipline(id) {
		api
			.getInstructorsByDiscipline(auth, id)
	...
	function handleNewTest(e) {
		...
		api
			...
			.catch(() => {
				Swal.fire({
					icon: 'error',
					text: 'Não foi possível adicionar a prova',
				});
				setLoading(false);
			});
	}
